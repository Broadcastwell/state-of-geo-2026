...
**Three of the four API accounts ran out of credit during collection. One was topped up and finished; two were not.** The study as executed therefore holds 18 ChatGPT, 280 Claude, 175 Perplexity, 280 Google AI Overviews answers out of 280 planned for each. Section 3.8 states exactly what stopped and when. Every figure below carries the sample it was computed on, and the four-engine intersection is reported at its true size of 18 questions rather than dressed up. The two engines that completed, Claude and Google AI Overviews, are the two that carry the repeat comparison below.
...
**Three, and this is why the paper still stands:** the decisive test in section 4.5 does not need four engines. It needs repeats, and both engines that finished have them. Google AI Overviews and Claude each completed all three runs of all 25 stability questions, contributing 62 and 74 repeat pairs respectively. The two engines that ran out of credit contribute no repeat pairs and no verdict, which is stated rather than hidden.

The Layer 2 extractor shares the Anthropic key with the Claude engine, so it stopped when that account did and resumed with it. It covers 100.0% of collected answers (808 of 808). Layer 1 covers every answer. Because Layer 2 can only ADD out-of-universe names, the gap can only narrow a vendor set and never widen one, and every universe-only statistic in this paper is completely independent of it. The universe-only sensitivity analysis in section 4.8 is therefore also the Layer-2-free reading of the results.
